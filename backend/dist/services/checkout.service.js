...
async function createCheckoutOrder(input) {
    ...
    const result = await pool_1.pool.query('SELECT id, price, in_stock, stock_quantity FROM products WHERE id = ANY($1::int[])', [ids]);
    ...
    const quantityMap = new Map(result.rows.map((row) => [Number(row.id), Number(row.stock_quantity)]));
    ...
    let discountRate = 0;
    if (input.promoCode) {
        const coupon = await pool_1.pool.query(`SELECT discount_percent, min_subtotal
       FROM coupons
       WHERE code = $1
         AND active = true
         AND (expires_at IS NULL OR expires_at > now())`, [input.promoCode]);
        const foundCoupon = coupon.rows[0];
        if (!foundCoupon) {
    ...
    if (bundleRule) {
        ...
        if (qualifies) {
            const bundleSubtotal = input.items.reduce((sum, item) => {
                if (!bundleRule.productIds.includes(item.id))
                    return sum;
                const price = priceMap.get(item.id);
                if (price === undefined)
                    return sum;
    ...
    for (const item of input.items) {
       ...
       VALUES ($1, $2, $3, $4)`, [orderId, item.id, Math.max(1, item.quantity), unitPrice]);
        await pool_1.pool.query(`UPDATE products
       SET stock_quantity = GREATEST(0, stock_quantity - $2),
           in_stock = (stock_quantity - $2) > 0
       WHERE id = $1`, [item.id, Math.max(1, item.quantity)]);
    }
