...
async function getWishlistProducts(userId) {
    const result = await pool_1.pool.query(`SELECT p.id, p.name, p.brand, p.category, p.flavor, p.servings, p.price, p.description, p.image, p.images,
...
async function addWishlistProduct(userId, productId) {
    await pool_1.pool.query(`INSERT INTO wishlist_items (user_id, product_id)
     VALUES ($1, $2)
     ON CONFLICT (user_id, product_id) DO NOTHING`, [userId, productId]);
    return getWishlistProducts(userId);
}
async function removeWishlistProduct(userId, productId) {
    await pool_1.pool.query('DELETE FROM wishlist_items WHERE user_id = $1 AND product_id = $2', [userId, productId]);
    return getWishlistProducts(userId);
}
async function getAdminProducts() {
    const result = await pool_1.pool.query(`SELECT id, name, brand, category, flavor, servings, price, description, image, images, in_stock as "inStock",
...
async function createAdminProduct(input) {
    const result = await pool_1.pool.query(`INSERT INTO products (id, name, brand, category, flavor, servings, price, description, image, images, ingredients, usage, faqs, reviews, goals, in_stock, stock_quantity, low_stock_threshold, featured, supplement_facts, certifications)
...
async function getAdminCoupons() {
    const result = await pool_1.pool.query(`SELECT id, code, description, discount_percent as "discountPercent", min_subtotal as "minSubtotal",
...
async function createAdminCoupon(input) {
    const result = await pool_1.pool.query(`INSERT INTO coupons (code, description, discount_percent, min_subtotal, active, expires_at)
     VALUES ($1, $2, $3, $4, $5, $6)
...
async function updateAdminCoupon(id, input) {
    const result = await pool_1.pool.query(`UPDATE coupons SET
     ...
     WHERE id = $1
     RETURNING id, code, description, discount_percent as "discountPercent", min_subtotal as "minSubtotal",
               active, expires_at as "expiresAt", created_at as "createdAt"`, [id, input.description, input.discountPercent, input.minSubtotal, input.active, input.expiresAt]);
    return result.rows[0];
}
